...
def System(command):
    ...
    def volume_up():
        keyboard.press_and_release("volume up")  # Simulate the vol
...
async def TranslateAndExecute(commands: list[str]):
    ...
    # **FIX:** Gather and return results directly instead of yielding
    return await asyncio.gather(*funcs)


# Asynchronous function to automate command execution.
async def Automation(commands: list[str]):
    await TranslateAndExecute(commands)  # Just await the function instead of using async for
    return True


if __name__ == "__main__":
    asyncio.run(Automation(["open word", "open instagram", "open telegram", "play afsanay"]))